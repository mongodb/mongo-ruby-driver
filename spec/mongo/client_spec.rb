...
describe Mongo::Client do
  ...
  describe '#spawn_mongocryptd' do
    ...
    context 'without auto encryption options' do
      let(:client) { new_local_client_nmio([SpecConfig.instance.addresses.first]) }

      it 'raises an exception' do
        expect { client.spawn_mongocryptd }.to raise_error(ArgumentError, 'Cannot spawn mongocryptd process without setting auto encryption options on the client.')
      end
    end

    context 'with auto encryption options' do
      let(:extra_options) { {} }
      let(:pid) { client.spawn_mongocryptd }
