  ...
  describe '#aggregate_spec' do
    ...
    context 'when the collection has a read preference' do
      ...
      it 'includes the read preference in the spec' do
        ...
        expect(aggregation.send(:aggregate_spec, server)[:read]).to eq(read_preference)
      end
    end
    ...
    context 'when allow_disk_use is set' do
      ...
      it 'includes the option in the spec' do
        expect(aggregation.send(:aggregate_spec, server)[:selector][:allowDiskUse]).to eq(true)
      end
      ...
      context 'when allow_disk_use is specified as an option' do
        ...
        context 'when #allow_disk_use is also called' do
          ...
          it 'overrides the first option with the second' do
            expect(aggregation.send(:aggregate_spec, server)[:selector][:allowDiskUse]).to eq(false)
          end
        end
      end
    end
    ...
    context 'when max_time_ms is an option' do
      ...
      it 'includes the option in the spec' do
        expect(aggregation.send(:aggregate_spec, server)[:selector][:maxTimeMS]).to eq(options[:max_time_ms])
      end
    end
    ...
    context 'when batch_size is set' do
      ...
      context 'when batch_size is set on the view' do
        ...
        it 'uses the batch_size on the view' do
          expect(aggregation.send(:aggregate_spec, server)[:selector][:cursor][:batchSize]).to eq(view_options[:batch_size])
        end
      end
      ...
      context 'when batch_size is provided in the options' do
        ...
        it 'includes the option in the spec' do
          expect(aggregation.send(:aggregate_spec, server)[:selector][:cursor][:batchSize]).to eq(options[:batch_size])
        end
    ...
    context 'when a hint is specified' do
      ...
      it 'includes the option in the spec' do
        expect(aggregation.send(:aggregate_spec, server)[:selector][:hint]).to eq(options['hint'])
      end
    end
      ...
      context 'when use_cursor is true' do
        ...
        context 'when batch_size is not set' do
          ...
          it 'sets an empty document in the spec' do
            expect(aggregation.send(:aggregate_spec, server)[:selector][:cursor]).to eq({})
          end
        end
      ...
      context 'when use_cursor is false' do
        ...
        context 'when batch_size is set' do
          ...
          it 'does not set the cursor option in the spec' do
            expect(aggregation.send(:aggregate_spec, server)[:selector][:cursor]).to be_nil
          end
        end
      end
    end
