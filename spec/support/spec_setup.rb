...
class SpecSetup
  def run
    ...
    begin
      ...
      begin
        ...
        if e.message =~ /command createUser requires authentication|there are no users authenticated|not authorized on admin to execute command.*createUser/
          ...
          # exception
          if admin_authorized_test_client.use('admin').database.users.info(SpecConfig.instance.root_user.name).any?
      ...
      ClientRegistry.instance.global_client('admin_unauthorized').close
    end
    ...
    begin
      ...
      ClientRegistry.instance.global_client('root_authorized').close
    end
  end
