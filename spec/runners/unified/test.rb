  ...
  class Test
    ...
    def generate_entities(es)
      ...
      es.each do |entity_spec|
        raise NotImplementedError, 'Entity must have exactly one key' unless entity_spec.keys.length == 1
                     ...
                     if oe = observe_events
                       oe.each do |event|
                         ...
                         when /\A(?:pool|connection)/
                           ...
                           kind = event.sub('Event', '').gsub(/([A-Z])/) do
                             "_#{::Regexp.last_match(1)}"
                           end.sub('pool', 'Pool').downcase.to_sym
                           subscriber.add_wanted_events(kind)
        ...
        raise NotImplementedError, "Unhandled spec keys: #{spec}" unless spec.empty?
    ...
    def set_initial_data
      @spec['initialData']&.each do |entity_spec|
        ...
        begin
          ...
          if e.is_a?(Mongo::Error::OperationFailure::Family) && (
    ...
    def run
      ...
      raise NotImplementedError, "Unhandled spec keys: #{test_spec}" unless test_spec.empty?
    ensure
      disable_fail_points
    end
      ...
      use_all(op, 'operation', op) do |op|
        ...
        if expected_error = op.use('expectError')
          begin
            ...
            if expected_error.use('isTimeoutError') && !e.is_a?(Mongo::Error::TimeoutError)
              raise e
              raise Error::ErrorMismatch, %,Expected TimeoutError ("isTimeoutError") but got #{e},
            end
            # isClientError doesn't actually mean a client error.
            # It means anything other than OperationFailure. DRIVERS-1799
            if expected_error.use('isClientError') && e.is_a?(Mongo::Error::OperationFailure::Family)
              raise Error::ErrorMismatch, %,Expected not OperationFailure ("isClientError") but got #{e},
            end
            if (code = expected_error.use('errorCode')) && !(e.code == code)
            ...
            raise NotImplementedError, "Unhandled keys: #{expected_error}" unless expected_error.empty?
          ...
          if expected_result = op.use('expectResult')
            ...
            elsif expected_result.is_a?(Array)
    ...
    def use_all(_hash, key, v)
      orig_v = v.dup
      (yield v).tap do
        raise NotImplementedError, "Unconsumed items for #{key}: #{v}\nOriginal hash: #{orig_v}" unless v.empty?
      end
    end
    ...
    def disable_fail_points
      return unless $disable_fail_points
    ...
    def create_client(**opts)
      args = case v = options[:client_args]
             when Array
               raise NotImplementedError, 'Client args array must have two elements' unless v.length == 2
             ...
             when String
               ...
               addresses = [ addresses.first ] if options[:single_address]
               [
                 addresses,
