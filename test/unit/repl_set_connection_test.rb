  ...
  context "Initialization: " do
    context "connecting to a replica set" do
      setup do
        ...
        admin_db.stubs(:command).returns({'ok' => 1, 'ismaster' => 1, 'hosts' => @hosts, 'arbiters' => @arbiters})
    ...
    context "initializing with a mongodb uri" do
      ...
      should "parse a uri specifying multiple nodes" do
        ...
        assert_equal Node.new(['localhost', 27017]), @conn.nodes[0]
        assert_equal Node.new(['mydb.com', 27018]), @conn.nodes[1]
      end
    end
  end
end
