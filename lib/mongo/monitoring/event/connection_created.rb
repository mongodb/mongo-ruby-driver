...
module Mongo
  class Monitoring
    module Event
      ...
      #
      # @since 2.7.0
      class ConnectionCreated < Mongo::Event::Base

        # @return [ String ] address The address of the server the connection will connect to.
        #
        # @since 2.7.0
        attr_reader :address
        ...
        def summary
          "#<#{self.class.name.sub(/^Mongo::Monitoring::Event::/, '')} " +
              "connection_id=#{connection_id} address=#{address}>"
        end
      end
    end
  end
