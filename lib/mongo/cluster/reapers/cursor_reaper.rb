        ...
        to_kill_copy.each do |server, op_specs|
          op_specs.each do |op_spec|
            server.with_connection do |connection|
              command_class = if connection.features.find_command_enabled?
                Cursor::Builder::KillCursorsCommand
              else
                Cursor::Builder::OpKillCursors
              end

              command_class.update_cursors(op_spec, active_cursors_copy.to_a)
              if command_class.get_cursors_list(op_spec).size > 0
